...
// GitHub OAuth App ids are 20 hex chars; GitHub App ids are `Iv1.` + 16 hex.
const CLIENT_ID_RE = /^(?:[0-9a-fA-F]{20}|Iv1\.[0-9a-fA-F]{16})$/
  ...
  },
  {
    ...
    handle: async (input) => {
      ...
      if (!CLIENT_ID_RE.test(clientId)) {
        return { error: 'Client ID 格式不正确。请填写 GitHub OAuth App 的 20 位十六进制 Client ID（或 GitHub App 的 Iv1.xxx Client ID）。' }
      }
