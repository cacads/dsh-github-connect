...
const DEFAULT_AUTH_FILE = join(PLUGIN_DIR, '.github-auth.json')
// Where the token lives. Point this OUTSIDE the plugin directory: pnpm rewrites
// `node_modules/<plugin>/` wholesale on every install (`dsh plugin add/update`), and
// any file the plugin wrote there is deleted with it. Override with either the
// DSH_GITHUB_AUTH_FILE env var or the loader row's `authFile` config.
let authFile = (process.env.DSH_GITHUB_AUTH_FILE ?? '').trim() || DEFAULT_AUTH_FILE
...
export function apply(ctx, config = {}) {
  ...
  // Credentials may live outside the plugin directory (see `authFile` above).
  if (typeof config?.authFile === 'string' && config.authFile.trim() !== '') {
    authFile = config.authFile.trim()
  }
